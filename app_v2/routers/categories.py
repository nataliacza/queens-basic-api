...
from helpers import (
    is_unique_name,
    delete_tag_from_queens_db,
    update_tag_name_in_queens_db,
)
...
async def update_category(*, category_id: UUID, update_data: CategorySave):
    ...
    get_item = categories_db.get(encoded_id)
    if get_item:
        check = is_unique_name(
            fraze=update_data.name, given_data=categories_db.fetch().items
        )
        if not check:
            return JSONResponse(
                status_code=400,
